    ...
    const mockResults = [
        {
            ...
            dangerous: false,
            confidence: 85,
            summary: "This appears to be a harmless garden snake. These snakes are non-venomous and generally beneficial for pest control.",
            medicalAttention: false,
            advice: "Garden snakes are completely harmless to humans. If bitten (which is rare), simply clean the wound with soap and water.",
            emergencyLevel: "none"
        },
        {
            ...
            dangerous: true,
            confidence: 92,
            summary: "This appears to be a Black Widow spider, which is venomous and potentially dangerous.",
            medicalAttention: true,
            advice: "If bitten by a Black Widow, seek immediate medical attention. Apply ice to the bite area and try to remain calm.",
            emergencyLevel: "high"
        },
        {
            animal: "Honey Bee",
            dangerous: true,
            confidence: 78,
            summary: "This appears to be a honey bee. While generally not aggressive, they can sting if threatened.",
            medicalAttention: false,
            advice: "If stung, remove the stinger by scraping (not pinching), apply ice, and monitor for allergic reactions.",
            emergencyLevel: "low"
        }
    ];
...
function displayResults(result) {
    ...
    resultCard.innerHTML = `
        <div class="result-header">
            <h4><i class="${iconClass}" style="color: ${iconColor}; margin-right: 8px;"></i>${result.animal}</h4>
...
function generateBotResponse(userMessage) {
    ...
    if (message.includes('bee') || message.includes('wasp') || message.includes('sting')) {
    ...
    if (message.includes('upload') || message.includes('photo') || message.includes('picture')) {
...
function askFollowUp() {
    ...
    chatInput.focus();
    addBotMessage("I see you'd like to ask a follow-up question about the analysis. What would you like to know more about?");
}
...
function clearMedicalChat() {
    if (confirm('Are you sure you want to clear the medical chat history?')) {
        ...
        const initialMessage = medicalChatMessages.querySelector('.medical-bot-message');
...
function downloadMedicalChat() {
    ...
    chatContent += 'Always consult with healthcare professionals for medical advice.';
...
function generateMedicalBotResponse(userMessage) {
    ...
    if (message.includes('allergic') && (message.includes('reaction') || message.includes('bee') || message.includes('sting'))) {
    ...
    if (message.includes('wound') || message.includes('clean') || message.includes('care')) {
    ...
    if (message.includes('emergency room') || message.includes('er') || message.includes('hospital')) {
...
function callPoisonControl() {
    if (confirm('This will attempt to call Poison Control (1-800-222-1222). Continue?')) {
...
function startTelemedicine() {
    alert('Telemedicine integration would connect to your healthcare provider\'s telemedicine platform. This is a demo feature.');
}
